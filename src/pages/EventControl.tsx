...
export default function EventControl() {
  ...
  const [event, setEvent] = useState<Show | null>(null); const [game, setGame] = useState<SportsGame | null>(null); const [loaded, setLoaded] = useState(false); const [participantCount, setParticipantCount] = useState(0); const [totalJoinedCount, setTotalJoinedCount] = useState(0); const [starting, setStarting] = useState(false); const [copiedId, setCopiedId] = useState(false);
  ...
  async function startShow() {
    ...
    // Give Firebase and audience phones time to receive the start command
    // before the shared start instant. The soundtrack uses that same instant.
    const startTime = serverNow() + (delay + 5) * 1000;
    ...
    try {
      ...
      audio.volume = 1;
      await updateShow(eventId, {
        status: 'running',
        showStartTime: startTime,
        showStartOffset: offset,
        lightTimeline: generatedTimeline,
      });

      const playAtSharedStart = () => {
        startTimerRef.current = null;
        const elapsedSeconds = Math.max(0, (serverNow() - startTime) / 1000);
        // If the organizer tab was delayed or the write took longer than
        // expected, seek forward instead of starting the song behind the lights.
        audio.currentTime = Math.min(Math.max(0, offset + elapsedSeconds), Math.max(0, songDuration - 0.01));
        setSongCurrentTime(audio.currentTime);
        void audio.play().catch(error => {
          console.error('Could not play show audio:', error);
          setAnalysisMessage('The show is synchronized, but browser audio playback was blocked. Press Play on the audio player to start the soundtrack.');
        });
      };

      const schedulePlayback = () => {
        const remainingMs = startTime - serverNow();
        if (remainingMs <= 0) {
          playAtSharedStart();
          return;
        }
        startTimerRef.current = window.setTimeout(schedulePlayback, remainingMs);
      };

      const updateCountdown = () => {
        const remaining = Math.max(0, Math.ceil((startTime - serverNow()) / 1000));
        setCountdown(remaining > 0 ? remaining : null);
        if (remaining > 0) countdownTimerRef.current = window.setTimeout(updateCountdown, Math.min(250, Math.max(50, startTime - serverNow())));
        else countdownTimerRef.current = null;
      };
  ...
  return <main className="ls-shell ls-event-shell" style={{ '--ls-accent': homeColor, '--ls-accent-2': awayColor, '--ls-accent-ink': getReadableTextColor(homeColor) } as React.CSSProperties}>
    <header className="ls-header ls-event-header"><div><div className="ls-brand">FANCOURT360</div><p className="ls-eyebrow">SPORTS EVENT CONTROL</p></div><button type="button" className="ls-button ls-secondary" onClick={() => navigate('/admin')}>BACK TO EVENTS</button></header>
    ...
    <section className="ls-show-id-card"><div><span className="ls-eyebrow">EVENT ID</span><strong>{eventId}</strong></div><button type="button" className="ls-copy-id" onClick={() => void copyEventId()} aria-label="Copy event ID">{copiedId ? 'COPIED' : 'COPY ID'}</button></section>
    <div className="ls-group"><div className="ls-group-heading"><span className="ls-group-bar" /><div><h2>Overview</h2><span>Access &amp; live results</span></div></div><div className="ls-overview-grid">
      <div className="ls-card ls-qr-card"><div className="ls-section-title"><div><p className="ls-eyebrow">AUDIENCE ACCESS</p><h2>Scan to Join</h2></div></div><button type="button" onClick={() => window.open(`/audience/${eventId}`, '_blank', 'noopener,noreferrer')} style={{ width: '100%', cursor: 'pointer', border: 0, background: 'transparent', padding: 0 }} aria-label="Open QR audience screen"><div className="ls-qr-frame"><div className="ls-qr-corner" /><QRCodeSVG value={joinUrl} size={220} bgColor="#ffffff" fgColor="#050505" level="H" /></div></button><p className="ls-qr-url">{joinUrl}</p><p className="ls-muted ls-qr-help">Click the QR code to open the full-screen audience display.</p><button type="button" className="ls-button ls-primary" style={{ width: '100%', marginTop: 14 }} onClick={() => window.open(`/audience/${eventId}`, '_blank', 'noopener,noreferrer')}>OPEN AUDIENCE SCREEN</button></div>
      ...
      <div className="ls-card"><p className="ls-eyebrow">EVENT ANALYTICS</p><h2>Audience results</h2><div className="ls-analytics-list"><div className="ls-analytics-row"><span className="ls-muted">TOTAL JOINED</span><strong>{stats.totalJoined}</strong></div><div className="ls-analytics-row"><span className="ls-muted">PEAK CONNECTED</span><strong>{stats.peakConnected}</strong></div></div></div>
    </div></div>

...
export default function Join() {
  ...
  useEffect(() => watchServerTimeOffset(() => { if (!joined || !event) return; if (event.status === 'running' && event.showStartTime && event.lightTimeline) synchronizeShow(event.showStartTime, event.lightTimeline as LightTimeline, event.showStartOffset ?? 0); }), [joined, event?.status, event?.showStartTime, event?.showStartOffset, event?.lightTimeline]);
